...
class dice : public eosio::contract { //继承contract基类
  private:
    ...
    struct offer                //定义数据库表 offer
    {
        ...
        name owner;     //账户数据类型 name，参考 https://developers.eos.io/eosio-cpp/reference#name
    ...
    typedef eosio::multi_index<"offer"_n, offer,
                               indexed_by<"bet"_n, const_mem_fun<offer, uint64_t, &offer::by_bet>>,
                               indexed_by<"commitment"_n, const_mem_fun<offer, key256, &offer::by_commitment>>>
    ...
    struct account //参与账户类
    {
        ...
        uint64_t primary_key() const { return owner.value; } //用户名为主键，不能同时进行两个游戏
 
        EOSLIB_SERIALIZE(account, (owner)(eos_balance)(open_offers)(open_games))
    };
 
    typedef eosio::multi_index<"account"_n, account> account_index;  //定义一个账户类型：account_index
                       ...
                       const offer &loser_offer)
    {
        ...
        accounts.modify(winner_account, "0"_n, [&](auto &acnt) {
        ...
        accounts.modify(loser_account, "0"_n, [&](auto &acnt) {
            acnt.open_games--;
        });
  ...
  public:
    ...
    void withdraw(const name to, const asset &quantity)
    {
        ...
        action(
            permission_level{_self, "active"_n}, //BUG：应该改为 permission_level{_self, N(eosio.code)}
            "eosio.token"_n, "transfer"_n,
